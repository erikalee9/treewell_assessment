...
persb_totalcumTreeTemp_corr_plot_abla <- ggplot(persb_corr_data_abla, aes(x = totalcum_treetemp, y = well_diam_m, color = factor(tree_id))) +
  ...
  theme_bw() +
  scale_color_manual(values = treeid_colors) + 
  scale_x_continuous(labels = scales::label_comma(), limits = c(225, 375), breaks = (breaks = seq(250, 350, by = 50))) + 
  #remove color legend 
  guides(color = "none") +
  scale_shape_manual(name = NULL, values = c("GB" = 16, "DB" = 15)) +
...
```{r}
PSZ_totalcum_NR <- ggarrange(persb_totalcumNR_corr_plot_accu, persb_totalcumNR_corr_plot_abla, nrow = 2, ncol = 1, common.legend = TRUE, legend = "none")
...
```{r}
TSZ_totalcum_TreeTemp <- ggarrange(transb_totalcumTreeTemp_corr_plot_accu, transb_totalcumTreeTemp_corr_plot_abla, nrow = 2, ncol = 1, common.legend = TRUE, legend = "bottom")
...
transb_totalcumNR_corr_plot_abla <- ggplot(transb_corr_data_abla, aes(x = totalcum_NR, y = well_diam_m, color = factor(tree_id))) +
  ...
  theme_bw() +
  scale_color_manual(values = treeid_colors) + 
  scale_x_continuous(labels = scales::label_comma()) +  # Ensure numbers are not in scientific notation
  #remove color legend 
  guides(color = "none") +
  scale_shape_manual(name = NULL, values = c("GB" = 16, "DB" = 15)) +
...
```{r}
TSZ_totalcum_NR <- ggarrange(transb_totalcumNR_corr_plot_accu, transb_totalcumNR_corr_plot_abla, nrow = 2, ncol = 1, common.legend = TRUE, legend = "bottom")
...
```{r}
#NR
comb_corr_NR<- ggarrange(PSZ_totalcum_NR, TSZ_totalcum_NR, nrow = 2, ncol = 1, common.legend = TRUE, legend = "none")
...
```{r}
cum_persb_corr_data_accu <- persb_corr_data_accu %>%
  group_by(tree_id) %>%
  arrange(tree_id, date) %>%  # Ensure data is ordered by date within each tree
  mutate(cum_AirTC_Avg = cumsum(AirTC_Avg)) %>%
  select(-time_mst) %>%
  mutate(NetSW_MJperd = SWnet_Avg * 86.4, NetLW_MJperd = LWnet_Avg * 86.4, NR_MJperd = NR_Avg * 86.4) %>%
  mutate(cum_SW_MJ = cumsum(NetSW_MJperd), cum_LW_MJ = cumsum(NetLW_MJperd), cum_NR_MJ = cumsum(NR_MJperd))
```
...
p_SD_r2_values_abla_V2 <- persb_corr_data_abla_V2 %>%
  ...
  group_by(tree_id) %>%
  summarize(R2 = cor(well_diam_m, maximum_depth_m, use = "complete.obs")^2, .groups = "drop") %>%
  mutate(R2_label = paste0("R² = ", formatC(R2, format = "f", digits = 2)))
...
persb_vol_SWE_fullseason_lineplot <- ggplot() + 
  ...
  geom_line(data = fullseason_vol_persb, aes(x = date, y = vol_sensor_m3, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
...
transb_vol_SWE_fullseason_lineplot <- ggplot() + 
  ...
  geom_line(data = fullseason_vol_transb, aes(x = date, y = vol_sensor_m3, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
...
```{r}
ggsave("comb_fullseason_SWEvol.png", height = 5, width = 8, plot = comb_fullseason_SWEvol, dpi = 600)
```
...
PSZ_MRD_MAD_stats <- fullseason_vol_persb %>%
  ...
  summarise(
    ...
    mean_rel_diff =mean(abs(vol_sensor_m3 - vol_tree_m3) / ((vol_sensor_m3 + vol_tree_m3) / 2), na.rm = TRUE), 
    min_avg_diff = min(abs(vol_sensor_m3 - vol_tree_m3), na.rm = TRUE),
    min_rel_diff =min(abs(vol_sensor_m3 - vol_tree_m3) / ((vol_sensor_m3 + vol_tree_m3) / 2), na.rm = TRUE),
    max_avg_diff = max(abs(vol_sensor_m3 - vol_tree_m3), na.rm = TRUE),
    max_rel_diff =max(abs(vol_sensor_m3 - vol_tree_m3) / ((vol_sensor_m3 + vol_tree_m3) / 2), na.rm = TRUE)
```
