...
```{r}
transb_fullseason_treewell_diams_plot <- ggplot() +
  # Tree well diameter lines
  geom_line(data = long_transb_treewell_diams_fullseason, 
            aes(x = date, y = well_diam, color = factor(tree_id))) +
  # Shortwave and longwave incoming radiation as bars
  geom_col(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = SWin_Avg * scaling_factor), fill = "orange", alpha = 0.5) + 
  #geom_col(data = filtered_daily_transb_wx_15min_r, 
          # aes(x = date, y = LWin_Avg), fill = "blue", alpha = 0.5) + 
  
  # Air temperature as a line
 # geom_line(data = filtered_daily_transb_wx_15min_r, 
           # aes(x = date, y = AirTC_Avg), color = "black", size = 1) +
  
  # Scale for primary and secondary axes
  scale_y_continuous(
    name = "Well Diameter (cm)", 
    sec.axis = sec_axis(~ . / scaling_factor, name = "Radiation (W/m²)")
  ) +
  
  scale_color_manual(values = c("1" = "red", "2" = "blue", "3" = "green", "4" = "purple")) +  
  labs(x = "Date", color = "Tree ID") +
  theme_minimal()


transb_fullseason_treewell_diams_plot
```
