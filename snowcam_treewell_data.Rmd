...
```{r}
#Daily mean SWE - using Joe Wright and Hourglass predicted snow density data (V2)
...
SWEonly_fullseason_vol_persb <- vol_persb_daily_wx_fullseason %>%
  ...
  mutate(
    SWE_sensor_cm = if_else(row_number() == n(), 0, SWE_sensor_cm),
    SWE_sensor_cm_smooth = if_else(row_number() == n(), 0, SWE_sensor_cm_smooth)
  )
...
persb_SWE_tree1 <- SWE_fullseason_vol_persb %>%
  filter(tree_id == "1", vol_tree_m3 >= 0) %>%
  select(date, tree_id, vol_tree_cm3:last_col())
...
#dataframe to use for open area SWE line
SWE_persb_wx_fullseason <- vol_persb_daily_wx_fullseason %>%
  inner_join(persb_mean_daily_SWE_V2, by = "date") %>%
  mutate(
  SWE_sensor_cm = ((vol_sensor_cm3 / 1e6) * predicted_snowdensity_kg_m3) / (1.7671 * 1000) * 100
) %>%
...
#dataframe to use for open area SWE line
SWE_transb_wx_fullseason <- vol_transb_daily_wx_fullseason %>%
  inner_join(transb_mean_daily_SWE_V2, by = "date") %>%
  mutate(
  SWE_sensor_cm = ((vol_sensor_cm3 / 1e6) * predicted_snowdensity_kg_m3) / (1.7671 * 1000) * 100
) %>%
  mutate(SWE_sensor_cm_smooth = rollmean(SWE_sensor_cm, k = 10, fill = NA, align = "center"))
...
persb_SWE_fullseason_lineplot_V2 <- ggplot() + 
  # Line denoting ablation season
  geom_vline(xintercept = as.Date('2024-05-06'), color = "grey23", linetype = "dashed", linewidth = 0.6, alpha = 0.7) +
  geom_line(data = SWE_persb_wx_fullseason, aes(x = date, y = SWE_sensor_cm_smooth, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
...
```{r}
transb_SWE_fullseason_lineplot_V2 <- ggplot() + 
  # Line denoting ablation season
  geom_vline(xintercept = as.Date('2024-05-06'), color = "grey23", linetype = "dashed", linewidth = 0.6, alpha = 0.7) +
  geom_line(data = SWE_transb_wx_fullseason, aes(x = date, y = SWE_sensor_cm_smooth, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = transb_SWE_tree1_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 1", linetype = "tree 1"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = transb_SWE_tree2_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 2", linetype = "tree 2"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = transb_SWE_tree3_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 3", linetype = "tree 3"), linewidth = 0.8, alpha = 0.8) +
  geom_line(data = transb_SWE_tree4_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 4", linetype = "tree 4"), linewidth = 0.8, alpha = 0.8) +  
  facet_wrap(~ tree_id, nrow = 1, ncol = 4) +  
  labs(
    title = "b) TSZ",
    x = NULL,
...
transb_SWE_fullseason_lineplot_V2
