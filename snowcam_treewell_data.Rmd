...
filtered_daily_transb_wx_15min_r <- filtered_transb_wx_15min %>%
  ...
  arrange(date) %>%
  mutate(totalcum_AirTC = cumsum(AirTC_Avg), totalcum_SWnet = cumsum(SWnet_Avg), totalcum_LWnet = cumsum(LWnet_Avg), totalcum_NR = cumsum(NR_Avg))
```
...
persb_totalcumTreeTemp_corr_plot_abla <- ggplot(persb_corr_data_abla, aes(x = totalcum_treetemp, y = well_diam, color = factor(tree_id))) +
  ...
  theme_bw() +
  scale_color_manual(values = treeid_colors) + 
  scale_x_continuous(labels = scales::label_comma())+  #remove color legend
  #remove color legend 
  guides(color = "none") +
  scale_shape_manual(name = "Burn Status", values = c("GB" = 16, "DB" = 15)) +
