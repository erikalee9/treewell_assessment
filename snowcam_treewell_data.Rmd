...
filtered_daily_transb_wx_15min_r <- filtered_transb_wx_15min %>%
  ...
  filter(phase == "day") %>%
  #converting radiation values to MJ per day
  mutate(SWin_Avg = SWin_Avg * 86.4, LWin_Avg = LWin_Avg * 86.4, SWout_Avg = SWout_Avg * 86.4, LWout_Avg = LWout_Avg * 86.4, SWnet_Avg = SWnet_Avg * 86.4, LWnet_Avg = LWnet_Avg * 86.4, NR_Avg_new = NR_Avg_new * 86.4) %>%
  group_by(date) %>%
  summarize(
  ...
  arrange(date) %>%
  mutate(totalcum_AirTC = cumsum(cum_AirTC), totalcum_SWnet = cumsum(cum_SWnet), totalcum_LWnet = cumsum(cum_LWnet), totalcum_NR = cumsum(cum_NR))
```
...
persb_daily_treetemp_data <- full_15min_ns %>%
  ...
  arrange(date) %>%
  mutate(totalcum_treetemp = cumsum(cum_daily_tree_temp))
```
...
persb_totalcumNR_corr_plot_accu <- ggplot(persb_corr_data_accu, aes(x = totalcum_NR, y = well_diam, color = factor(tree_id))) +
  ...
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "PSZ: Accumulation, Cumulative Net Rad",
    x = "Total Cumulative Net Radiation (MJ)",
    y = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
  theme_bw() +
  scale_color_manual(values = treeid_colors) + 
  scale_x_continuous(labels = scales::label_number()) +  # Ensure numbers are not in scientific notation
  #remove color legend 
  guides(color = "none") +
  scale_shape_manual(name = "Burn Status", values = c("GB" = 16, "DB" = 15)) +
...
persb_totalcumNR_corr_plot_abla <- ggplot(persb_corr_data_abla, aes(x = totalcum_NR, y = well_diam, color = factor(tree_id))) +
  ...
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "PSZ: Ablation, Cumulative Net Rad",
    x = "Total Cumulative Net Radiation (MJ)",
    y = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
...
transb_totalcumTreeTemp_corr_plot_accu <- ggplot(transb_corr_data_accu, aes(x = totalcum_treetemp, y = well_diam, color = factor(tree_id))) +
  ...
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "TSZ: Accumulation, Total Cumulative Tree Temperature",
    x = "Total Cumulative Tree Temperature (°C)",
    y = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
...
transb_totalcumTreeTemp_corr_plot_abla <- ggplot(transb_corr_data_abla, aes(x = totalcum_treetemp, y = well_diam, color = factor(tree_id))) +
  ...
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "TSZ: Ablation, Total Cumulative Tree Temperature",
    x = "Total Cumulative Tree Temperature (°C)",
    y = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
...
transb_totalcumNR_corr_plot_accu <- ggplot(transb_corr_data_accu, aes(x = totalcum_NR, y = well_diam, color = factor(tree_id))) +
  ...
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "TSZ: Accumulation, Total Cumulative NR",
    x = "Total Cumulative Net Radiation (MJ)",
    y = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
...
transb_totalcumNR_corr_plot_abla <- ggplot(transb_corr_data_abla, aes(x = totalcum_NR, y = well_diam, color = factor(tree_id))) +
  ...
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "TSZ: Ablation, Total Cumulative NR",
    x = "Total Cumulative Net Radiation (MJ)",
    y = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
