...
filtered_daily_transb_wx_15min_r <- filtered_transb_wx_15min %>%
  filter(day >= "2024-02-01" & day <= "2024-05-08") %>%
  filter(phase == "day") %>%
  group_by(date) %>%
  summarize(
...
transb_fullseason_well_SW_plot <- ggplot() +
  ...
  geom_col(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = SWin_Avg * scaling_factor_SW), fill = "orange", alpha = 0.5) + 
# Shortwave out
  geom_col(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = SWout_Avg * scaling_factor_SW), fill = "red", alpha = 0.4) + 
...
transb_fullseason_well_LW_plot <- ggplot() +
  ...
  # Shortwave in 
  geom_col(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = LWin_Avg * scaling_factor_LW), fill = "dodgerblue1", alpha = 0.5) + 
# Shortwave out
  geom_col(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = LWout_Avg * scaling_factor_LW), fill = "dodgerblue3", alpha = 0.4) + 
     # Tree well diameter lines
  geom_line(data = long_transb_treewell_diams_fullseason, 
            aes(x = date, y = well_diam, color = factor(tree_id), linetype = burn_status), linewidth = 0.8) +  
  # Y-axis scaling
...
transb_fullseason_well_airTCRad_plot <- ggplot() +
 ...
 # geom_vline(data = transb_treewell_reset_dataframe, 
  ...
  # NR Avg
  geom_col(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = NR_Avg), fill = "orange", alpha = 0.5) + 
    # Air Temp 
  geom_line(data = filtered_daily_transb_wx_15min_r, 
           aes(x = date, y = AirTC_Avg * scaling_factor_airT), color = "dodgerblue3", linewidth = 0.8, alpha = 0.8) + 
  # Tree well diameter lines
  geom_line(data = long_transb_treewell_diams_fullseason, 
            aes(x = date, y = well_diam, color = factor(tree_id), linetype = burn_status), linewidth = 0.8) +  
    #points for treewell resets
  geom_point(data = transb_treewell_reset_dataframe, 
           aes(x = date, y = well_diam), 
           shape = 16, color = "black", size = 1.5) + 
  # Y-axis scaling
  scale_y_continuous(
  ...
  scale_shape_manual(
    name = NULL,
    values = c("Dates that wells were reset with new snow" = 16)) + 
