...
\*\* still need to plot cumulative values in TSZ, then combine with PSZ!
...
```{r}
#rough well volume calculations for 30 trees in a 30 square meter area
# Parameters
area_m2 <- 30 * 30           # Total area in m²
...
persb_SWE_fullseason_lineplot <- ggplot() + 
  ...
  geom_line(data = SWEonly_fullseason_vol_persb, aes(x = date, y = SWE_sensor_kgm3, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
...
transb_SWE_fullseason_lineplot <- ggplot() + 
  ...
  geom_line(data = SWEonly_fullseason_vol_transb, aes(x = date, y = SWE_sensor_kgm3, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
...
New SWE from only Joe Wright and Hourglass
...
Adding snow density data to snow volume

-   need to use area and volume of cylinder to convert snow volume to SWE

```{r}
SWE_fullseason_vol_persb_V2 <- fullseason_vol_persb %>%
  inner_join(persb_mean_daily_SWE_V2, by = c("date", "tree_id")) %>%
  mutate(
  SWE_tree_cm = ((vol_tree_cm3 / 1e6) * predicted_snowdensity_kg_m3) / (1.7671 * 1000) * 100, 
  SWE_sensor_cm = ((vol_sensor_cm3 / 1e6) * predicted_snowdensity_kg_m3) / (1.7671 * 1000) * 100
)
```

```{r}
SWE_fullseason_vol_transb_V2 <- fullseason_vol_transb %>%
  inner_join(transb_mean_daily_SWE_V2, by = c("date", "tree_id")) %>%
  mutate(
  SWE_tree_cm = ((vol_tree_cm3 / 1e6) * predicted_snowdensity_kg_m3) / (1.7671 * 1000) * 100, 
  SWE_sensor_cm = ((vol_sensor_cm3 / 1e6) * predicted_snowdensity_kg_m3) / (1.7671 * 1000) * 100
)
...
SWEonly_fullseason_vol_persb_V2 <- SWE_fullseason_vol_persb_V2 %>%
  filter(SWE_sensor_cm >= 0) %>%
  mutate(SWE_sensor_cm_smooth = rollmean(SWE_sensor_cm, k = 10, fill = NA, align = "center"))

persb_SWE_tree1_V2 <- SWE_fullseason_vol_persb_V2 %>%
  filter(tree_id == "1", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))
  
persb_SWE_tree2_V2 <- SWE_fullseason_vol_persb_V2 %>%
  filter(tree_id == "2", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))

persb_SWE_tree3_V2 <- SWE_fullseason_vol_persb_V2 %>%
  filter(tree_id == "3", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))

persb_SWE_tree4_V2 <- SWE_fullseason_vol_persb_V2 %>%
  filter(tree_id == "4", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))
```

```{r}
SWEonly_fullseason_vol_transb_V2 <- SWE_fullseason_vol_transb_V2 %>%
  filter(SWE_sensor_cm >= 0) %>%
  mutate(SWE_sensor_cm_smooth = rollmean(SWE_sensor_cm, k = 10, fill = NA, align = "center"))

transb_SWE_tree1_V2 <- SWE_fullseason_vol_transb_V2 %>%
  filter(tree_id == "1", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))
  
transb_SWE_tree2_V2 <- SWE_fullseason_vol_transb_V2 %>%
  filter(tree_id == "2", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))

transb_SWE_tree3_V2 <- SWE_fullseason_vol_transb_V2 %>%
  filter(tree_id == "3", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))

transb_SWE_tree4_V2 <- SWE_fullseason_vol_transb_V2 %>%
  filter(tree_id == "4", SWE_tree_cm >= 0) %>%
  mutate(SWE_tree_cm_smooth = rollmean(SWE_tree_cm, k = 10, fill = NA, align = "center"))
```
...
```{r}
persb_SWE_fullseason_lineplot_V2 <- ggplot() + 
  # Line denoting ablation season
  geom_vline(xintercept = as.Date('2024-05-06'), color = "grey23", linetype = "dashed", linewidth = 0.6, alpha = 0.7) +
  geom_line(data = SWEonly_fullseason_vol_persb_V2, aes(x = date, y = SWE_sensor_cm_smooth, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = persb_SWE_tree1_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 1", linetype = "tree 1"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = persb_SWE_tree2_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 2", linetype = "tree 2"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = persb_SWE_tree3_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 3", linetype = "tree 3"), linewidth = 0.8, alpha = 0.8) +
  geom_line(data = persb_SWE_tree4_V2, aes(x = date, y = SWE_tree_cm_smooth, color = "tree 4", linetype = "tree 4"), linewidth = 0.8, alpha = 0.8) +  
  facet_wrap(~ tree_id, nrow = 1, ncol = 4) +  
  labs(
    title = "a) PSZ",
    x = NULL,
...
persb_SWE_fullseason_lineplot_V2
...
```{r}
persb_tree_well_corr_plot_abla_V2 <- ggplot(comb_persb_treewell_diam_abla_V2, aes(x = tree_diam_m, y = welldiam_minustree_m, color = factor(tree_id))) +
  geom_point(aes(shape = burn_status), size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", color = "grey", se = TRUE) +
  facet_wrap(~ date) +  # Separate by tree_id
  labs(
    title = "a) PSZ",
    x = "Tree Diameter (m)",
    y = "Well Diameter (minus tree diam) (m)",
    color = "Tree ID"
  ) +
  theme_bw() +
  scale_color_manual(values = treeid_colors) + 
  ...
  guides(color = "none") +
  scale_shape_manual(name = NULL, values = c("GB" = 16, "DB" = 15)) +
  geom_text(data = p_treewelldiam_camera_results_V2, aes(x = Inf, y = Inf, label = R2_label), 
            hjust = 1.1, vjust = 1.5, inherit.aes = FALSE, size = 4) +
  theme(
    strip.text = element_text(size = 11),  # Facet title size
    axis.title = element_text(size = 11),  # Axis title size
...
persb_tree_well_corr_plot_abla_V2
```
...
```{r}
transb_tree_well_corr_plot_abla_V2 <- ggplot(comb_transb_treewell_diam_abla_V2, aes(x = tree_diam_m, y = welldiam_minustree_m, color = factor(tree_id))) +
  geom_point(aes(shape = burn_status), size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", color = "grey", se = TRUE) +
  facet_wrap(~ date) +  # Separate by tree_id
  labs(
  ...
  guides(color = "none") +
  scale_shape_manual(name = NULL, values = c("GB" = 16, "DB" = 15)) +
  geom_text(data = t_treewelldiam_camera_results_V2, aes(x = Inf, y = Inf, label = R2_label), 
            hjust = 1.1, vjust = 1.5, inherit.aes = FALSE, size = 4) +
  theme(
    strip.text = element_text(size = 11),  # Facet title size
    axis.title = element_text(size = 11),  # Axis title size
...
transb_tree_well_corr_plot_abla_V2
```

Combining V2 plots

```{r}
comb_snowcam_treevswell_corr_abla_V2 <- ggarrange(persb_tree_well_corr_plot_abla_V2, transb_tree_well_corr_plot_abla_V2, nrow = 2, ncol = 1, common.legend = TRUE, legend = "bottom")

comb_snowcam_treevswell_corr_abla_V2
```

```{r}
ggsave("comb_snowcam_treevswell_corr_abla_appendix_V2.png", height = 10, width = 7.5, plot = comb_snowcam_treevswell_corr_abla_V2, dpi = 600)
```

### Subtracting Tree Diam from Well Diam for snow cam data, by tree ID
...
NOT Subtracting, for comparison!

PSZ

```{r}
tree1_persb_treevswelldiam_corr_results_V1 <- tree1_persb_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree2_persb_treevswelldiam_corr_results_V1 <- tree2_persb_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree3_persb_treevswelldiam_corr_results_V1 <- tree3_persb_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree4_persb_treevswelldiam_corr_results_V1 <- tree4_persb_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree1_transb_treevswelldiam_corr_results_V1<- tree1_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree2_transb_treevswelldiam_corr_results_V1<- tree2_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree3_transb_treevswelldiam_corr_results_V1<- tree3_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
...
```{r}
tree4_transb_treevswelldiam_corr_results_V1<- tree4_treevswelldiam_V2 %>%
  summarize(
    cor_test = list(tryCatch(cor.test(well_diam_m, tree_diam_m, use = "complete.obs"), 
                             error = function(e) return(NA))), 
    .groups = "drop"
