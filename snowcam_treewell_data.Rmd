...
TSZ_largearea_wellvol_plot <- ggplot() + 
  ...
  geom_line(data = TSZ_largearea_vol_data, aes(x = date, y = total_vol_tree_m3, color = "tree well", linetype = "tree well"), linewidth = 0.8, alpha = 0.8) + 
  labs(
    title = NULL,
    x = "Date",
    y = "Snow Volume (m³)",
...
PSZ_largearea_vol <- persb_col_tree2 %>%
  #anything with L is for this large area calculations
   mutate(
    L_well_diam_m = well_diam/100, 
    L_well_rad_m = L_well_diam_m/2, 
     # Daily well volume around one tree, 
    L_tree_vol_m3 = (2/3) * pi * (L_well_rad_m^3), 
     #total area snow volume for 35x35m area
    L_openarea_vol_m3 = area_m2 * (maximum_depth_cm/100), 
    # well volume by tree, as applied to well diam data by day
    L_total_well_vol_m3 = n_trees * L_tree_vol_m3,
    # Remaining snow volume in open areas with tree volume subtracted
    L_remainingsnow_vol_m3 = L_openarea_vol_m3 - L_total_well_vol_m3
  ) %>%
  mutate(
    L_openarea_vol_m3 = if_else(row_number() == n(), 0, L_openarea_vol_m3),
    L_remainingsnow_vol_m3 = if_else(row_number() == n(), 0, L_remainingsnow_vol_m3)
  ) %>%
  mutate(
    total_vol_tree_m3 = vol_tree_m3*n_trees, 
    total_openarea_vol_m3 = area_m2 * (maximum_depth_cm/100), 
    remaining_snowvol_m3 = total_openarea_vol_m3 - total_vol_tree_m3)

TSZ_largearea_vol <- transb_col_tree1 %>%
  #anything with L is for this large area calculations
   mutate(
    L_well_diam_m = well_diam/100, 
    L_well_rad_m = L_well_diam_m/2, 
    ...
    L_tree_vol_m3 = (2/3) * pi * (L_well_rad_m^3), 
     #total area snow volume for 35x35m area
    L_openarea_vol_m3 = area_m2 * (SnowDepth_cm/100), 
    # well volume by tree, as applied to well diam data by day
    L_total_well_vol_m3 = n_trees * L_tree_vol_m3,
    # Remaining snow volume in open areas with tree volume subtracted
    L_remainingsnow_vol_m3 = L_openarea_vol_m3 - L_total_well_vol_m3
...
```{r}
PSZ_largearea_wellvol_plot <- ggplot() + 
  # Line denoting ablation season
  geom_vline(xintercept = as.Date('2024-05-06'), color = "grey23", linetype = "dashed", linewidth = 0.6, alpha = 0.7) +
  geom_line(data = PSZ_largearea_vol, aes(x = date, y =
                                            remaining_snowvol_m3, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = PSZ_largearea_vol, aes(x = date, y = total_vol_tree_m3, color = "tree well", linetype = "tree well"), linewidth = 0.8, alpha = 0.8) + 
  labs(
    title = "a) PSZ",
    x = "Date",
    y = "Snow Volume (m³)",
...
TSZ_largearea_wellvol_plot <- ggplot() + 
  # Line denoting ablation season
  geom_vline(xintercept = as.Date('2024-04-01'), color = "grey23", linetype = "dashed", linewidth = 0.6, alpha = 0.7) +
  geom_line(data = TSZ_largearea_vol, aes(x = date, y = remaining_snowvol_m3, color = "open area", linetype = "open area"), linewidth = 0.8, alpha = 0.8) + 
  geom_line(data = TSZ_largearea_vol, aes(x = date, y = total_vol_tree_m3, color = "tree well", linetype = "tree well"), linewidth = 0.8, alpha = 0.8)
