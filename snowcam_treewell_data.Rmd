...
filtered_daily_transb_wx_15min_r <- filtered_transb_wx_15min %>%
  filter(day >= "2024-02-01" & day <= "2024-05-23") %>%
  filter(phase == "day") %>%
  group_by(date) %>%
  summarize(
...
# AirTC Correlation
# Compute R^2 for each tree_id
p_AirTC_r2_values_V2 <- persb_corr_data_V2 %>%
...
# Scatterplot
persb_AirTC_corr_plot_V2 <- ggplot(persb_corr_data_V2, aes(x = AirTC_Avg, y = well_diam, color = factor(tree_id))) +
  geom_point() +  # Scatter points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Trendline
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "PSZ, Well Diameter vs. Air Temperature",
    y = "Air Temperature (°C)",
    x = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
  ...
  geom_text(data = p_AirTC_r2_values_V2, aes(x = Inf, y = Inf, label = R2_label), 
            hjust = 1.1, vjust = 1.5, inherit.aes = FALSE, size = 5)

persb_AirTC_corr_plot_V2
```

```{r}
# AirTC Correlation
# Compute R^2 for each tree_id
p_TreeTemp_r2_values_V2 <- persb_corr_data_V2 %>%
...
# Scatterplot
persb_TreeTemp_corr_plot_V2 <- ggplot(persb_corr_data_V2, aes(x = mean_tree_temp, y = well_diam, color = factor(tree_id))) +
  geom_point() +  # Scatter points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Trendline
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "PSZ, Well Diameter vs. Tree Temperature",
  ...
  theme_bw() +
  scale_color_manual(values = c("1" = "red", "2" = "blue", "3" = "orange", "4" = "purple")) +  # Fix color mapping
  geom_text(data = p_TreeTemp_r2_values_V2, aes(x = Inf, y = Inf, label = R2_label), 
            hjust = 1.1, vjust = 1.5, inherit.aes = FALSE, size = 5)

persb_TreeTemp_corr_plot_V2
```
...
# SWnet Correlation
# Compute R^2 for each tree_id
p_NetSW_r2_values_V2 <- persb_corr_data_V2 %>%
...
# Scatterplot
persb_NetSW_corr_plot_V2 <- ggplot(persb_corr_data_V2, aes(x = SWnet_Avg, y = well_diam, color = factor(tree_id))) +
  geom_point() +  # Scatter points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Trendline
  facet_wrap(~ tree_id) +  # Separate by tree_id
  labs(
    title = "PSZ, Well Diameter vs. Net SW",
    y = "Net SW (W/m^2)",
    x = "Well Diameter (cm)",
    color = "Tree ID"
  ) +
  theme_bw() +
  scale_color_manual(values = c("1" = "red", "2" = "blue", "3" = "orange", "4" = "purple")) +  # Fix color mapping
  geom_text(data = p_NetSW_r2_values_V2, aes(x = Inf, y = Inf, label = R2_label), 
            hjust = 1.1, vjust = 1.5, inherit.aes = FALSE, size = 5)

persb_NetSW_corr_plot_V2
```
