...
treewell_exploration_plot <- ggplot(data = treewell_Tapril_Pmay) +
  geom_point(aes(x = dbh, y = treewell_diameter, color = as.character(date)), size = 2) +  # Convert date to character
  geom_smooth(aes(x = dbh, y = treewell_diameter), method = 'lm', se = TRUE, color = "grey") +
  labs(
...
```{r}
ggsave(
  filename = "tree_vs_well_diam_manuelmeasurements_scatterplot.png",  # File name
