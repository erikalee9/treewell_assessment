...
treewell_n_long <- treewell_n %>%
  ...
  select(tree_ID, site, date, tree_mort, aspect, dbh, treewell_diameter_full, measurement, well_diameter, increments_cm)
...
trans_treewell_aspect_may_plot <- ggplot(data = trans_treewell_long_aspect_may) +
  geom_point(aes(x = dbh, y = well_diameter)) +
  theme_bw() +
  geom_smooth(aes(x = dbh, y = well_diameter), method = 'lm', se = TRUE, color = "blue") +
  labs(title = "Transitional", x = "Tree Diameter (cm)", y = "Well Diameter (cm)") +
  theme_bw() +  # Use a minimal theme for a clean look
  scale_color_manual(values = aspect_color, labels = color_labels) + 
  facet_wrap(~aspect) +
  coord_cartesian(ylim = c(0, 85), xlim = c(0,30))
...
pers_treewell_aspect_may_plot <- ggplot(data = pers_treewell_long_aspect_may) +
  ...
  theme_bw() +
  geom_smooth(aes(x = dbh, y = well_diameter), method = 'lm', se = TRUE, color = "blue") +
  labs(title = "Persistent", x = "Tree Diameter (cm)", y = "Well Diameter (cm)") +
  theme_bw() +  # Use a minimal theme for a clean look
  scale_color_manual(values = aspect_color, labels = color_labels) + 
  facet_wrap(~aspect) +
  coord_cartesian(ylim = c(0, 85), xlim = c(0,30))
...
library(ggpubr)
#arranging plots
comb_treewell_aspect_plot <- ggarrange(pers_treewell_aspect_may_plot, trans_treewell_aspect_may_plot, ncol = 2, nrow = 1, common.legend = TRUE, legend = "right")

comb_treewell_aspect_plot
```

```{r}
"Well Diameter by Aspect and Tree Diameter on 2024-05-03"
```
...
treewell_aspect_pointplot <- ggplot(data = treewell_aspect_dataframe) +
  geom_point(aes(x = dbh, y = d_edge_N), color = "d_edge_N") +
  geom_point(aes(x = dbh, y = d_edge_E), color = "d_edge_E") +
  geom_point(aes(x = dbh, y = d_edge_S), color = "d_edge_S") +
  geom_point(aes(x = dbh, y = d_edge_W), color = "d_edge_W") +
  theme_bw() +
