...
treewell_exploration_plot <- ggplot(data = treewell_Tapril_Pmay) +
  geom_point(aes(x = dbh, y = treewell_diameter, color = as.character(date))) +  # Convert date to character
  geom_smooth(aes(x = dbh, y = treewell_diameter), method = 'lm', se = TRUE, color = "grey") +
  labs(
...
```{r}
#scatterplot of well vs tree diam, for all data

full_tree_well_scatterplot <- ggplot(full_snowcam_treewell_data, aes(x = tree_diam_cm, y = well_diam_cm)) +
  geom_point(aes(color = zone), size = 3, alpha = 0.7) +  # Color by tree_id
...
# Display the plot
full_tree_well_scatterplot
```

```{r}
#trans scatterplot

transb_tree_well_scatterplot <- ggplot(transb_snowcam_treewell_data, aes(x = tree_diam_cm, y = well_diam_cm)) +
  geom_point(aes(color = zone), size = 3, alpha = 0.7) +  # Color by tree_id
  geom_smooth(method = "lm", color = "grey", se = TRUE) +  # Best-fit line with shaded CI
  #annotate("text", x = 25, y = 10, label = paste("R² = ", round(r_squared, 2)), size = 5, color = "black") +# Add R² value
  facet_wrap(~tree_id) +
...
# Display the plot
transb_tree_well_scatterplot
```

```{r}
#plot by date vs treewell
transb_tree_well_lineplot <- ggplot(transb_snowcam_treewell_data_edited) +
  geom_line(aes(x = date, y = well_diam_cm)) +
 # geom_point(aes(color = zone), size = 3, alpha = 0.7) +  # Color by tree_id
  #geom_smooth(method = "lm", color = "grey", se = TRUE) +  # Best-fit line with shaded CI
  #annotate("text", x = 25, y = 10, label = paste("R² = ", round(r_squared, 2)), size = 5, color = "black") +# Add R² value
  facet_wrap(~tree_id) +
...
```{r}
#persb_highmort
persb_highmort_tree_well_lineplot <- ggplot(persb_highmort_snowcam_treewell_data_edited) +
  geom_line(aes(x = date, y = well_diam_cm)) +
 # geom_point(aes(color = zone), size = 3, alpha = 0.7) +  # Color by tree_id
  #geom_smooth(method = "lm", color = "grey", se = TRUE) +  # Best-fit line with shaded CI
  #annotate("text", x = 25, y = 10, label = paste("R² = ", round(r_squared, 2)), size = 5, color = "black") +# Add R² value
...
```{r}
#persb_highmort
persb_lowmort_tree_well_lineplot <- ggplot(persb_lowmort_snowcam_treewell_data_edited) +
  geom_line(aes(x = date, y = well_diam_cm)) +
 # geom_point(aes(color = zone), size = 3, alpha = 0.7) +  # Color by tree_id
  #geom_smooth(method = "lm", color = "grey", se = TRUE) +  # Best-fit line with shaded CI
  #annotate("text", x = 25, y = 10, label = paste("R² = ", round(r_squared, 2)), size = 5, color = "black") +# Add R² value
